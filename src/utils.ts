...
async function getClientTags() {
  ...
  const clientTags = clients.map((client: any) => client.tags).flat() as string[]
...
export async function launchTaggedClient(launchCommand: string, tag: string, rules: string[] = [], matcher = /\w/) {
  const stringifiedRules = rules.join(";")
  await runCommand(`hyprctl "dispatch exec [${stringifiedRules}] ${launchCommand}"`)
  const pidGetterOutput = (await runCommand("ps aux --sort +start_time | tail -n 4")).split("\n")[0]

  // Raise error if matcher is not found
  if (!matcher.test(pidGetterOutput)) throw new Error("No PID found for the application.")
